...
export default function CourseClient({ id }: { id: string }) {
    const enrollment = MOCK_ENROLLMENTS.find((e) => e.course_id === id);
    ...
    function getCTAButton() {
        ...
        if (enrollment!.progress_percent === 0) return <Link href={`/course/${id}/identity-check`}><Button variant="primary" size="lg"><ArrowRight size={18} />Start Course</Button></Link>;
        return <Link href={`/course/${id}/identity-check`}><Button variant="primary" size="lg"><PlayCircle size={18} />Resume Course</Button></Link>;
    }
